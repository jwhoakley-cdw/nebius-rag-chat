...
#            llama_base64 = base64.b64encode(llama_file.read()).decode()
    ...
    with col1:
        assets_path = "./assets"
        qwen_b64, llama_b64 = "", ""
        for fname, var in [("qwen.png", "qwen_b64"), ("llamaindex.png", "llama_b64")]:
            fpath = os.path.join(assets_path, fname)
            if os.path.exists(fpath):
                with open(fpath, "rb") as f:
                    locals()[var]  # noqa — assigned below
                    if var == "qwen_b64":
                        qwen_b64 = base64.b64encode(f.read()).decode()
                    else:
                        llama_b64 = base64.b64encode(f.read()).decode()
...
#        )
    ...
    with st.sidebar:
        nebius_logo = os.path.join("./assets", "nebius.png")
        ...
        # FIX: Embedding model is now selectable; default is bge-multilingual-gemma2
        embedding_model = st.selectbox(
            "Embedding Model",
            EMBEDDING_MODELS,
            index=0,
            help=(
                "BAAI/bge-multilingual-gemma2 is recommended. "
                "The previously used BAAI/bge-en-icl has been retired by Nebius."
            )
        )
...
#                    st.error(f"Error: {str(e)}")
        ...
        with st.chat_message("assistant"):
            with st.spinner("Thinking..."):
                try:
                    # FIX: Pass the selected embedding_model from the sidebar
                    response = run_rag_completion(
                        st.session_state.documents,
                        prompt,
                        embedding_model,      # now correctly uses sidebar selection
                        generative_model
                    )
