...
# Load Azure Secrets
KEYVAULT_NAME = os.environ.get("AZJWHO_CDW_AI_KEYVAULT_NAME",)
...
def run_rag_completion(
    ...
    if not embedding_model or not generative_model:
        return "❌ **Error:** No model selected. Please choose models from the sidebar."
    ...
    # Inject system prompt into the QA template if one is provided
    if system_prompt:
        qa_template = PromptTemplate(
            f"{system_prompt}\n\n"
            "You have been given the following context from a document:\n"
            "---------------------\n"
            "{context_str}\n"
            "---------------------\n"
...
def run_web_search(
    ...
    if not generative_model:
        return "❌ **Error:** No generative model selected. Please choose one from the sidebar."
    ...
    web_context = "\n\n".join([
        f"Source: {r.get('url', '')}\n{r.get('content', '').strip()[:600]}"
        for r in results
    ])

    # If a system prompt is set, synthesise through the LLM
    if system_prompt:
        llm = NebiusLLM(
            model=generative_model,
            api_key=os.getenv("NEBIUS_API_KEY"),
        ...
        synthesis_prompt = (
            f"{system_prompt}\n\n"
            f"Using only the following information from the University of Wolverhampton website, "
            f"answer the question below. Cite the source URLs where relevant.\n\n"
            f"**Web Context:**\n{web_context}\n\n"
            f"**Question:** {query_text}"
        )
        return str(llm.complete(synthesis_prompt))

    # Fallback: return formatted raw results
    header = "🎓 *Results from **wlv.ac.uk** only*\n\n---\n\n"
    formatted = "\n\n---\n\n".join([
        f"**[{r.get('title', f'Result {i}')}]({r.get('url', '')})**\n{r.get('content','').strip()[:600]}"
...
def main():
    ...
    with st.sidebar:
        ...
        # Central point to set system prompt for all modes
        st.divider()
        st.subheader("🧠 System Prompt")
        system_prompt = st.text_area(
            label="Customise the assistant's behaviour:",
            value=(
                "You are a helpful university assistant for the University of Wolverhampton. "
                "You help students with clearing-related queries including course availability, "
                "entry requirements, and the application process. "
                "Always be clear, friendly, and factual. "
                "If you are unsure of something, say so rather than guessing."
            ),
            height=180,
            help="This prompt shapes how the assistant responds. Edit it to change the assistant's tone, focus, or constraints."
)
        st.divider()
