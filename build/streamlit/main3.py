...
def run_web_search(query_text: str, max_results: int = 5) -> str:
    """Run a Tavily web search and return a formatted answer with sources."""
    tavily_api_key = os.getenv("TAVILY_API_KEY")
    if not tavily_api_key:
        return "❌ **Error:** `TAVILY_API_KEY` is not set in your environment."

    tavily_tool = TavilyToolSpec(api_key=tavily_api_key)
    results = tavily_tool.search(query=query_text, max_results=max_results)

    if not results:
        return "No results found for your query."

    # Build a clean response from the returned Document objects
    formatted_parts = []
    for i, doc in enumerate(results, start=1):
        title = doc.metadata.get("title", f"Result {i}")
        url = doc.metadata.get("url", "")
        snippet = doc.text.strip()[:600]  # cap snippet length
...
def run_hybrid_completion(
    ...
    """
    Combine PDF RAG context with Tavily web results, then synthesise
    a single answer using the Nebius LLM.
    """
    # 1. Get PDF context
    llm = NebiusLLM(
        model=generative_model,
    ...
    index = VectorStoreIndex.from_documents(documents)
    pdf_response = index.as_query_engine(similarity_top_k=5).query(query_text)
    ...
    if tavily_api_key:
        tavily_tool = TavilyToolSpec(api_key=tavily_api_key)
        results = tavily_tool.search(query=query_text, max_results=max_web_results)
        web_snippets = [
            f"Source: {r.metadata.get('url', '')}\n{r.text.strip()[:400]}"
            for r in results
        ]
        web_context = "\n\n".join(web_snippets)

    # 3. Synthesise via LLM
    synthesis_prompt = (
        ...
        f"Please synthesise both sources into a single, clear, well-structured answer. "
        f"Indicate where information comes from (PDF or web) where relevant."
    )
...
def main():
    ...
    with col1:
        ...
        st.markdown(
            ...
            f"<h1 style='margin:0;'>Nebius RAG Chat {qwen_img} + LlamaIndex {llama_img}</h1>"
            f"</div>",
    ...
    with st.sidebar:
        ...
        if query_mode != "📄 PDF Only":
            ...
            if not tavily_key_set:
                st.warning("⚠️ `TAVILY_API_KEY` not found in environment.")
            max_web_results = st.slider(
                "Max web results", min_value=1, max_value=10, value=5
            )
        else:
            max_web_results = 5
